  ...
  return (
    <div className="font-inter bg-[#f8fafc] text-slate-800 h-screen overflow-hidden flex selection:bg-sky-500 selection:text-white">
      <main className="flex-1 flex flex-col h-full relative bg-dashboard-grid z-10 overflow-y-auto pb-20">
        ...
        {/* ── Header ── */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 flex items-center justify-between sticky top-0 z-40 shadow-sm flex-shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xs font-mono-custom text-sky-600">Audit Hub</span>
              <span className="text-xs text-slate-400">/</span>
              <span className="text-xs font-mono-custom text-slate-500">{data.id}</span>
            </div>
            <h2 className="font-mono-custom font-bold text-xl text-slate-800 tracking-tight flex items-center gap-3">
              {data.domain}
              {data.verified && (
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="bg-white hover:bg-slate-50 text-slate-600 border border-slate-300 font-mono-custom font-bold text-xs px-4 py-2 rounded-lg transition-colors flex items-center gap-2 shadow-sm"
            >
            ...
            <button 
              ...
              className="bg-slate-900 hover:bg-slate-800 text-white border border-slate-900 font-mono-custom font-bold text-xs px-4 py-2 rounded-lg transition-colors flex items-center gap-2 shadow-md"
            >
        ...
        <div className="p-8 max-w-[90rem] mx-auto w-full space-y-8">
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-3xl p-10 shadow-2xl relative overflow-visible">
             <div className="absolute top-0 right-0 w-64 h-64 bg-sky-500/10 rounded-full blur-3xl pointer-events-none" />
             <div className="relative z-10 flex flex-col items-center">
                <h3 className="font-mono-custom text-[10px] font-bold text-sky-400 uppercase tracking-[0.4em] mb-6">
                   Select Audit Target
                </h3>
                ...
                <div className="mt-6 flex items-center gap-6">
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] font-mono-custom text-slate-500 uppercase tracking-widest mb-1">Target IP</span>
                    <span className="text-xs font-mono-custom text-slate-300 font-bold">{data.ip}</span>
                  </div>
                  <div className="w-px h-8 bg-slate-800" />
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] font-mono-custom text-slate-500 uppercase tracking-widest mb-1">Last Audit</span>
                    <span className="text-xs font-mono-custom text-slate-300 font-bold">{data.timestamp}</span>
                  </div>
                </div>
             </div>
          </section>
            ...
            <div className="col-span-1 lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-8 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
              ...
              <div className="relative w-48 h-48 flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center mt-2">
                  <span className="font-mono-custom font-extrabold text-5xl text-slate-900">{data.score}</span>
                  <span className="font-mono-custom text-xs text-slate-400 uppercase tracking-widest mt-1">/ 100</span>
                </div>
              </div>

              <p className={`mt-6 text-center text-sm font-bold px-4 py-1.5 rounded-full border 
                ...
                {data.score < 50 ? "Critical Status - Action Required" : data.score < 80 ? "Elevated Risk - Review Needed" : "Secure Environment"}
              </p>
            </div>

            <div className="col-span-1 lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
              <h3 className="font-mono-custom font-bold text-sm text-slate-800 uppercase tracking-widest mb-6">
                Industry Compliance
              </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={stat.label} className={`bg-white border-l-4 ${stat.color.split(' ')[0]} border-y border-r border-slate-200 rounded-r-xl p-5 shadow-sm hover:shadow-md transition-shadow`}>
                  <span className={`inline-block ${stat.color.split(' ')[2]} ${stat.color.split(' ')[1]} text-[10px] font-mono-custom font-bold uppercase tracking-widest mb-2 px-2 py-0.5 rounded`}>
                    {stat.label}
                  </span>
                  <p className={`text-4xl font-mono-custom font-bold ${stat.color.split(' ')[1]}`}>{stat.val}</p>
                </div>
              ))}
            </div>

            <div className="lg:col-span-1 bg-sky-50 border border-sky-200 rounded-xl p-5 shadow-sm relative overflow-hidden">
              <h4 className="text-xs font-bold text-sky-800 uppercase tracking-widest mb-2 font-mono-custom">Business Impact</h4>
              <p className="text-xs text-sky-700 leading-relaxed relative z-10 italic">
                {data.score < 50 
          ...
          <div className="mt-12">
            <div className="flex items-center gap-3 mb-6">
              <h3 className="font-amarillo text-3xl text-slate-900 drop-shadow-sm uppercase">
                Technical <span className="text-sky-500">Findings</span>
              </h3>
              <span className="bg-slate-900 text-white text-[10px] font-mono-custom px-3 py-1 rounded-full uppercase tracking-widest">
            ...
            <div className="space-y-8">
              {data.findings.length > 0 ? (
                data.findings.map((f) => (
                  <div key={f.id} className="bg-white border border-slate-200 rounded-2xl shadow-md overflow-hidden flex flex-col lg:flex-row">
                    <div className="w-full lg:w-1/2 border-r border-slate-100 flex flex-col">
                      <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <SeverityBadge severity={f.severity} />
                          <span className="font-mono-custom text-xs text-slate-600 bg-white border border-slate-200 px-2 py-0.5 rounded">
                            {f.cwe}
                          </span>
                        </div>
                        <span className="font-mono-custom text-xs text-slate-400 font-bold uppercase">{f.tool}</span>
                      </div>
                      <div className="p-6 flex-1">
                        <h4 className="font-bold text-slate-900 text-xl mb-2">{f.title}</h4>
                        <p className="text-sm text-slate-600 mb-6 leading-relaxed">{f.description}</p>
                        ...
                        <div className="bg-[#0f172a] rounded-xl border border-slate-800 overflow-hidden shadow-inner">
                          <div className="bg-[#1e293b] px-4 py-2 border-b border-slate-800 flex items-center gap-2">
                            <span className="font-mono-custom text-[10px] text-slate-400 uppercase tracking-widest">Evidence Log</span>
                          </div>
                          <div className="p-4 font-mono-custom text-xs overflow-x-auto terminal-scroll">
                    ...
                    <div className="w-full lg:w-1/2 p-6 bg-slate-50/50 flex flex-col justify-center">
                      <div className="inline-flex items-center gap-2 bg-green-100 border border-green-200 text-green-800 px-3 py-1.5 rounded-lg mb-4 w-fit shadow-sm">
                        <span className="font-mono-custom text-[10px] font-bold uppercase tracking-widest">Remediation Protocol</span>
                      </div>
                      <p className="text-sm text-slate-600 mb-4 font-bold">{f.remediation}</p>
                      ...
                      {f.codeSnippet && (
                        <div className="bg-slate-900 rounded-xl overflow-hidden shadow-md">
                          <div className="flex justify-between items-center px-4 py-2 border-b border-slate-700 bg-slate-800/80">
                            <span className="font-mono-custom text-[10px] text-slate-400">{f.filePath || "Suggested Patch"}</span>
                          </div>
                          <div className="p-4 font-mono-custom text-xs overflow-x-auto terminal-scroll">
                ...
                <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-20 text-center">
                   ...
                   <h4 className="font-bold text-slate-400 font-mono-custom uppercase tracking-widest text-sm">No Vulnerabilities Recorded</h4>
                   <p className="text-slate-400 text-xs mt-2 font-mono-custom">Clean audit path. No technical findings detected for this node.</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* ── Footer CTA ── */}
          <div className="mt-16 bg-slate-900 rounded-3xl p-1 relative overflow-hidden shadow-2xl">
            ...
            <div className="bg-slate-950 rounded-[1.4rem] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
              <div className="flex-1 text-center md:text-left">
                <h3 className="font-amarillo text-3xl text-white tracking-widest mb-2">AUTOMATE YOUR <span className="text-sky-500">DEFENSES</span></h3>
                <p className="text-slate-400 font-mono-custom text-sm max-w-xl leading-relaxed mb-4">
                  Stop scanning manually. Integrate Onyxspire directly into your CI/CD pipeline.
                </p>
              </div>
              <button className="bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-mono-custom font-bold text-sm px-8 py-4 rounded-xl shadow-[0_0_20px_rgba(14,165,233,0.4)] transition-transform transform hover:-translate-y-1">
                Generate API Keys
              </button>
            </div>
          </div>
